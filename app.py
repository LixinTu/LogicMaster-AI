...
# ========== Week 5: Page-rendering functions ==========

def _render_analytics_page():
    """Analytics 页面：A/B 测试结果 + RAG 性能指标"""
    st.header("Analytics")

    # --- A/B Test Results ---
    st.subheader("A/B Test Results")
    try:
        ab_resp = http_requests.get(
            f"{API_BASE_URL}/api/analytics/ab-test-results",
            params={"experiment": "tutor_strategy"},
            timeout=5,
        )
        if ab_resp.ok:
            ...
            if variants:
                ...
                # Accuracy by variant bar chart
                accuracies = []
                exposures = []
                for v in variant_names:
                    metrics = variants[v].get("metrics", {})
                    acc_vals = metrics.get("is_correct", {})
                    accuracies.append(acc_vals.get("mean", 0) * 100)
                    exposures.append(variants[v].get("exposures", 0))

                col_acc, col_exp = st.columns(2)
                with col_acc:
                    fig_acc = go.Figure(data=[go.Bar(
                        x=variant_names, y=accuracies,
                        marker_color=["rgb(32,201,151)", "rgb(99,110,250)"][:len(variant_names)],
                    )])
                    fig_acc.update_layout(title="Accuracy by Variant (%)", height=300,
                                         yaxis=dict(range=[0, 100]))
                    st.plotly_chart(fig_acc, use_container_width=True)

                with col_exp:
                    fig_exp = go.Figure(data=[go.Bar(
                        x=variant_names, y=exposures,
                        marker_color=["rgb(32,201,151)", "rgb(99,110,250)"][:len(variant_names)],
                    )])
                    fig_exp.update_layout(title="Exposures by Variant", height=300)
                    st.plotly_chart(fig_exp, use_container_width=True)

                # Expandable metrics table per variant
                for v in variant_names:
                    with st.expander(f"Variant: {v}"):
                        v_data = variants[v]
                        st.metric("Exposures", v_data.get("exposures", 0))
                        metrics = v_data.get("metrics", {})
                        for m_name, m_vals in metrics.items():
                            st.caption(f"**{m_name}**: mean={m_vals.get('mean', 0):.3f}, "
                                       f"std={m_vals.get('std', 0):.3f}, n={m_vals.get('count', 0)}")
            else:
                st.info("No A/B test data yet. Complete some questions to generate data.")
        else:
            st.warning(f"Failed to fetch A/B results (HTTP {ab_resp.status_code})")
    except Exception as e:
        st.warning(f"Could not load A/B test results: {e}")
    ...
    try:
        rag_resp = http_requests.get(
            f"{API_BASE_URL}/api/analytics/rag-performance",
            timeout=5,
        )
        if rag_resp.ok:
            rag_data = rag_resp.json()
            col_r1, col_r2, col_r3 = st.columns(3)
            col_r1.metric("Indexed Questions", rag_data.get("indexed_questions", "N/A"))
            col_r2.metric("Embedding Model", rag_data.get("embedding_model", "N/A"))
            col_r3.metric("Dimensions", rag_data.get("embedding_dims", "N/A"))

            quality = rag_data.get("quality_comparison")
            if quality and isinstance(quality, dict):
                rag_score = quality.get("rag_mean", 0)
                baseline_score = quality.get("baseline_mean", 0)
                if rag_score > 0 or baseline_score > 0:
                    fig_q = go.Figure(data=[go.Bar(
                        x=["RAG-Enhanced", "Baseline"],
                        y=[rag_score, baseline_score],
                        marker_color=["rgb(32,201,151)", "rgb(200,200,200)"],
                    )])
                    fig_q.update_layout(title="Explanation Quality (RAG vs Baseline)", height=300)
                    st.plotly_chart(fig_q, use_container_width=True)
        else:
            st.warning(f"Failed to fetch RAG performance (HTTP {rag_resp.status_code})")
    except Exception as e:
        st.warning(f"Could not load RAG performance: {e}")


def _render_learning_path_page():
    """Learning Path 页面：占位 + 技能概览"""
    st.header("Learning Path")
    st.info("Learning Path feature is coming soon. This will provide personalized study plans based on your skill profile.")

    questions_log = st.session_state.get("questions_log", [])
    if questions_log:
        st.subheader("Skill Profile Preview")
        skill_stats = {}
        ...
        if skill_stats:
            for skill, stats in sorted(skill_stats.items()):
                st.caption(f"**{skill}**: {stats['correct']}/{stats['total']} correct")


def _render_settings_page():
    """Settings 页面：API 配置 + A/B 实验信息 + 会话信息"""
    st.header("Settings")
    ...
    if api_key:
        ...
        st.success(f"DeepSeek API Key: `{masked}`")
    else:
        st.warning("DeepSeek API Key: Not set (enter in sidebar)")
    ...
    for exp_name in ["tutor_strategy", "explanation_source"]:
        try:
            ...
            if resp.ok:
                ...
                total_exp = sum(v.get("exposures", 0) for v in variants.values())
                st.caption(f"**{exp_name}**: {len(variants)} variants, {total_exp} total exposures")
            else:
                st.caption(f"**{exp_name}**: No data")
        except Exception:
            st.caption(f"**{exp_name}**: Unavailable (API offline)")

    st.divider()

    # Current session info
    st.subheader("Current Session")
    st.caption(f"**User ID**: `{st.session_state.get('user_id', 'N/A')}`")
    st.caption(f"**A/B Variant**: `{st.session_state.get('ab_variant') or 'Not assigned'}`")
    st.caption(f"**Questions Completed**: {st.session_state.get('attempt_count', 0)}")
    st.caption(f"**Current Theta**: {st.session_state.get('user_theta', 0.0):.2f}")
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["Practice", "Analytics", "Learning Path", "Settings"],
        key="nav_page",
        label_visibility="collapsed",
    )

    # 2. System Status（单次 /health 调用，派生 3 个指标）
    st.divider()
    st.subheader("System Status")
    _health_data = None
    try:
        _health_resp = http_requests.get(f"{API_BASE_URL}/health", timeout=2)
        if _health_resp.ok:
            _health_data = _health_resp.json()
    except Exception:
        pass

    if _health_data:
        st.success("API: Online")
        _db = _health_data.get("db_status", "disconnected")
        if _db == "connected":
            st.success("Database: Connected")
        else:
            st.error("Database: Disconnected")
        _qd = _health_data.get("qdrant_status", "disconnected")
        if _qd == "connected":
            st.success("RAG/Qdrant: Connected")
        else:
            st.error("RAG/Qdrant: Disconnected")
    else:
        st.error("API: Offline")
        st.error("Database: Unknown")
        st.error("RAG/Qdrant: Unknown")
...
if page == "Practice":
    ...
    with col1:
        ...
        if current_q:
                                    ...
                                    if result is None:
                                        ...
                                        try:
                                            ...
                                            if fallback_candidates and len(fallback_candidates) > 0:
                                                ...
                                                # 数据库为空，显示友好提示并保持当前题目
                                                st.warning("No questions in database. Please run `python generate_pool.py` to generate questions.")
                                        except Exception as e:
                                            # 数据库查询失败，显示错误信息
            ...
            if phase == "remediation":
                attempt = st.session_state.get("attempt", 0)
                current_q_id = st.session_state.get("current_q_id", "")
                st.info(f"There is an issue with your choice. Please answer the follow-up. Attempts: {attempt}/2")
    ...
    with col2:
                ...
                try:
                    if question_type in RULE_SKILL_POOL_BY_TYPE:
                        ...
                        if skills:
                            ...
                            st.warning("No skills to check")
                    else:
                        st.warning(f"Question type '{question_type}' not in rule pool mapping")
                except Exception as e:
                    st.warning(f"Error checking rule pool: {e}")
        ...
        try:
            ...
            st.warning(f"Skill radar failed: {e}")
            st.info("Complete questions to build skill profile")
